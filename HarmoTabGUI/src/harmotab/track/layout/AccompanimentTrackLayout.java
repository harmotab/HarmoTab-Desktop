...
/**
 * Positionnement graphiquement les �l�ments d'une piste d'accompagnement
 */
public class AccompanimentTrackLayout extends TrackLayout {
	...
	public AccompanimentTrackLayout(Track track) {
		super(track);
		m_accompanimentTrack = (AccompanimentTrack) track;
	}
	...
	public int getTrackHeight() {
		return ACCOMPANIMENT_TRACK_HEIGHT;
	}
	
	@Override
	public int getLayoutRound() {
		return m_accompanimentTrack.getLinkedTrack().getTrackLayout().getLayoutRound() + 1;
	}
	...
	public void processElementsPositionning(LocationList locations, int areaWidth, float scoreDuration) {
		...
		int linkedTrackId = m_track.getScore().getTrackId(m_accompanimentTrack.getLinkedTrack());
		...
		for (Element element : m_track) {
			// Recherche de la position de l'accord
			...
			if (startTimePoint != null) {
				...
				LocationItem accompanimentItem = LocationItem.newFromArea(element, 
		...
		// Ajout du EmptyArea pour finir la ligne courante
